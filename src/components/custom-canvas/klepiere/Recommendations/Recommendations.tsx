...
const Recommendations: FC<CustomComponentProps> = ({ component, context, slots, displayName, dealsPerPage = deals_PER_PAGE, dealsPage }) => {
    ...
    return (
        <div className="bg-white px-4 py-16 pb-20 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl">
                <h1 className="mb-8 text-center text-4xl font-bold text-gray-900 md:text-5xl lg:text-5xl">
                    Recommended deals
                </h1>
                ...
                <div className="flex justify-center gap-4 mb-8">
                    <button
                        ...
                        className="inline-flex items-center px-4 py-2 bg-[#166434] text-white rounded-md hover:bg-[#145c2f]"
                    >
                        {AUDIENCE_ICONS.americansignal} <span className="ml-2">I'm American</span>
                    </button>
                    <button
                        onClick={() => window.location.href = `${window.location.pathname}?country=spain`}
                        className="inline-flex items-center px-4 py-2 bg-[#3730A3] text-white rounded-md hover:bg-[#312b8a]"
                    >
                        {AUDIENCE_ICONS.spanishsignal} <span className="ml-2">I'm Spanish</span>
                    </button>
                    <button
                        onClick={() => window.location.href = window.location.pathname}
                        className="inline-flex items-center px-4 py-2 bg-[#991B1B] text-white rounded-md hover:bg-[#831818]"
                    >
                    ...
                    <div className="mt-8 flex justify-center">
                        <nav className="inline-flex border border-gray-300 shadow">
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                <button
                                    key={page}
                                    className={`px-4 py-2 text-sm font-medium ${page === currentPage ? `${currentPageColor} text-white` : 'bg-white text-gray-700 hover:bg-gray-50'
